...
from config import favor_voice, opposition_voice, judge_voice
...
def debate_func_for_gradio(statement):
    ...
    for i in range(3):
        ...
        for chunk in run_round(round=i+1, statement=statement, system_prompt=Opponent_A, model="gpt-4o", my_argues=a_accumulated[:-1], opo_argues=b_accumulated):
        ...
        time.sleep(get_approx_time(text=a_accumulated[-1]))

        b_accumulated.append("")
        for chunk in run_round(round=i+1, statement=statement, system_prompt=Opponent_B, model="gpt-4o", my_argues=b_accumulated[:-1], opo_argues=a_accumulated):
        ...
        time.sleep(get_approx_time(text=b_accumulated[-1]))
    ...
    time.sleep(get_approx_time(text=j_text))
